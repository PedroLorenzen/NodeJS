...
router.get("/jobs", async (req, res) => {
    if (req.session.user) {
        try {
            ...
            return res
                .status(404)
                .send({ message: "Jobs sorted by " + query + " not found." });
        } catch (error) {
    ...
    if (req.session.user) {
        try {
            ...
            if (!Number.isFinite(price) || price <= 0) {
                return res
                    .status(400)
                    .send({ error: "Price must be a number and over 0" });
            }
            if (!Number.isFinite(skill_id)) {
                return res
                    .status(400)
                    .send({ error: "Skill ID must be a number" });
            }
            if (!Number.isFinite(user_id)) {
                return res
                    .status(400)
                    .send({ error: "User ID must be a number" });
            }

            if (!name || !description) {
                return res
                    .status(400)
                    .send({ error: "Missing required information" });
            }

            const db = await connect();
            const generateJobId = await db
                .collection("counters")
                .findOneAndUpdate(
                    { _id: "jobId" },
    ...
    if (req.session.user) {
        ...
        if (!jobId) {
            return res.status(400).send({
                error: "Job ID must be provided as a query parameter",
            });
        }
        try {
            ...
            if (!Number.isFinite(jobId) || jobId <= 0) {
                return res
                    .status(400)
                    .send({ error: "Job ID must be a number and over 0" });
            }
            if (!Number.isFinite(skill_id) || skill_id <= 0) {
                return res
                    .status(400)
                    .send({ error: "Skill ID must be a number and over 0" });
            }
            if (!Number.isFinite(price) || price <= 0) {
                return res
                    .status(400)
