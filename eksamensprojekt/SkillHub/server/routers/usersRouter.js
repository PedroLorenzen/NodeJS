...
router.put("/users", async (req, res) => {
  if (req.session.user) {
    try {
      let { name, email, location, oldPassword, newPassword, isAdmin } =
        req.body;
      let user = {};
      let id = null;
      const db = await connect();
      ...
      if (req.query.getUserId) {
        user = await db
          .collection("users")
          .findOne({ _id: parseInt(req.query.getUserId) });
      ...
      if (user) {
        ...
        if (oldPassword) {
          if (
            ...
            return res.status(400).send({
              error:
                "Password must be between 6 and 70 characters long, include at least one uppercase letter, one special character, and not match the old password.",
            });
          }
