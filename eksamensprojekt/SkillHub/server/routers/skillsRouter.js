...
router.post("/skills", async (req, res) => {
    if (req.session.user.isAdmin) {
        try {
            ...
            const generateSkillId = await db
                .collection("counters")
                .findOneAndUpdate(
                    { _id: "skillId" },
    ...
    if (req.session.user.isAdmin) {
        ...
        if (!skillId) {
            return res.status(400).send({
                error: "Skill ID must be provided as a query parameter",
            });
        }
        try {
            let { name } = req.body;
...
router.delete("/skills", async (req, res) => {
    if (req.session.user.isAdmin) {
        ...
        try {
            const db = await connect();
            const jobsWithSkill = await db
                .collection("jobs")
                .find({ skill_id: skillId })
                .toArray();
            if (jobsWithSkill.length > 0) {
                return res.status(400).send({
                    error: "Cannot delete skill with associated jobs",
                });
            }
            ...
            if (deletedSkill.deletedCount === 1) {
                ...
                res.status(404).send({
                    error: "Skill not found or already deleted",
                });
            }
        } catch (error) {
            res.status(500).send({ error: "Database operation failed" });
