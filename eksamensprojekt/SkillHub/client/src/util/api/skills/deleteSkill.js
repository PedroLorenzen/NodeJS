...
export async function deleteSkill(skill, showToast) {
  ...
  if (response.status === 429) {
    ...
    toast.error(
      result.error ||
        "Failed to delete skill, check if there are jobs linked to the skill and try again",
      {
        duration: 3000,
        position: "top-right",
      },
...
export async function handleDeleteSkill(skill, showToast) {
  await toast.promise(
    deleteSkill(skill, showToast),
    {
      ...
      error:
        "Failed to delete skill - please try again - maybe there are still associated jobs?",
    },
    {
      duration: 2000,
      position: "top-right",
