<script>
  ...
  onMount(async () => {
    ...
    try {
      ...
      jobs = jobs.map((job) => ({
        ...job,
        skill_name: skills.find((skill) => skill.id === job.skill_id).name,
        user_id: job.user,
        user_name: users.find((user) => user.id === job.user_id).name,
        location: users.find((user) => user.id === job.user_id).location,
      }));
      jobs = jobs.filter((job) => job.user_id !== userId);
      jobs = jobs.sort((a, b) => a.skill_name.localeCompare(b.skill_name));
      allJobs = jobs;
      filterLocations();
    } catch (error) {
  ...
  function sortJobs() {
    ...
    if (sortBy === "skill_name") {
      ...
      jobs = allJobs;
      sortedJobs = jobs.sort((a, b) => a.location.localeCompare(b.location));
    }
  }
...
<main>
  ...
  <div class="actions">
    ...
    {#if filterBy === "skill_name"}
      ...
      <select bind:value={filterValue} on:change={filterJobs} id="skill_id">
    ...
    {#if filterBy === "location"}
      ...
      <select bind:value={filterValue} on:change={filterJobs} id="location">
    ...
    {#if filterBy === "user"}
      ...
      <select bind:value={filterValue} on:change={filterJobs} id="user_id">
          ...
          <div class="job-pair">
            ...
            {#if jobs[index + 1]}
              <div class="job">
                ...
                <button on:click={() => contactUser(jobs[index + 1].user_id)}
                  >Contact User</button
                >
              </div>
            {/if}
